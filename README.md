...
The repository includes three worked applications: `file-explorer` shows the TypeScript-only path, `activity-monitor` shows the same ordinary web-app structure gaining real operating-system access from an application-owned Rust extension, and `crumbbrot` uses a multi-core Rust extension for CPU-intensive fractal rendering. All three live under `examples/`, separate from the starter you edit.
...
Each render is also spread across every CPU core the machine offers. The extension splits the image into rows and hands them to a pool of scoped threads sized by `std::thread::available_parallelism`, drawing on nothing beyond the standard library. Workers claim one row at a time rather than taking fixed blocks, because rows crossing the interior of the set run to the full iteration limit while rows outside it escape after a few steps; a static split would strand most workers on cheap regions while one thread finished the expensive middle. The cancellation generation is re-checked before each row is claimed, so shutdown still interrupts a render promptly. On a 16-core Linux x64 machine this renders roughly ten times faster than the equivalent single-threaded loop, taking a 1600×1200 deep zoom at the 2,000-iteration limit from about 4.5 seconds to about 0.43 seconds. The crate's tests assert that the parallel output is byte-identical to a serial reference render for both fractal modes.
