...
What the template promises about the guidance it ships for coding assistants: that a canonical, vendor-neutral source exists, that installing it is one command and additive rather than destructive, that installed copies are verifiable against that source, and that a skill never becomes an authority competing with the documentation and specifications it summarizes.
