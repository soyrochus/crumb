...
All production filesystem access SHALL be owned by the Bun host and SHALL use inspection and bounded-read APIs only. Production code MUST NOT call filesystem creation, writing, copying, moving, renaming, deletion, permission, ownership, or link-creation APIs and MUST NOT invoke a shell.

#### Scenario: Static read-only review
- **WHEN** production source is searched for mutation and shell-execution APIs
