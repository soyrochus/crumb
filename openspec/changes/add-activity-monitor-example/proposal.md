...
- **Build and CI cost**: this is the repository's first application that needs Cargo, so building all applications now compiles Rust on both targets. That cost lands in the release workflow and is another argument for the deferred prebuilt-addon work.
